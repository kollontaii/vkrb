 ...
 * @param {Number} startNodeId optional - ID начальной точки
 * @returns {Promise<Graph>} Построенный граф
 */
export async function getMapGraph(boundingBox, startNodeId = null) {
    const response = await fetchOverpassData(boundingBox, false);
    const data = await response.json();
    ...
    for(const element of elements) {
        if(element.type === "node") {
            ...
            if(startNodeId && node.id === startNodeId) {
    ...
    // Если startNodeId не задан или не найден, просто выбираем первый узел в качестве начального
    if(!graph.startNode && graph.nodes.size > 0) {
        graph.startNode = graph.nodes.values().next().value;
    }
